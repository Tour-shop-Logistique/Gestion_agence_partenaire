...
export function useWebSocket(agenceId, handlers = {}, enabled = true) {
  ...
  const handleModelUpdate = useCallback((payload) => {
    ...
    switch (model) {
      ...
      case 'Announcement':
        console.log(`📢 [WebSocket] Annonce - Action: ${action}`);
        if (action === 'created' && handlersRef.current.onAnnouncementCreated) {
          console.log('✅ [WebSocket] Handler onAnnouncementCreated appelé');
          handlersRef.current.onAnnouncementCreated(data, { ids, references, changes, count, at });
        } else {
          console.warn(`⚠️ [WebSocket] Action '${action}' non gérée pour Announcement ou handler manquant`);
        }
        break;

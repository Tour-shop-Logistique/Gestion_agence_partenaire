...
const StatsCards = ({ logistics, operational }) => {
    return (
        <>
            {/* Top Destinations */}
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden col-span-1 md:col-span-1">
                            ...
                            return (
                                <div key={index} className="flex items-center justify-between group hover:bg-slate-50 -mx-2 px-2 py-1 sm:py-1.5 rounded-lg transition-colors">
                                    ...
                                    <div className="flex items-center gap-1 sm:gap-2">
                                        <span className="text-sm sm:text-base font-black text-slate-900">{dest.total}</span>
                                        <span className="text-[10px] sm:text-xs text-slate-400 font-medium">exp.</span>
                                    </div>
                                </div>
                            );
                        })
                            ...
                            return (
                                <div key={index} className="group">
                                    <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                                        <span className="text-[10px] sm:text-xs font-bold text-slate-700 group-hover:text-slate-900 transition-colors">
                                            {vol.type}
                                        </span>
                                        <div className="flex items-center gap-1 sm:gap-2">
                                            <span className="text-xs sm:text-sm font-black text-slate-900">{vol.total}</span>
                                            <span className={`text-[9px] sm:text-xs font-bold ${color.text} ${color.bg} px-1.5 sm:px-2 py-0.5 rounded-full`}>
                                    ...
                                    <div className="w-full bg-slate-100 rounded-full h-2 sm:h-2.5 overflow-hidden">
                                        <div 
                                            className={`${color.bar} h-2 sm:h-2.5 rounded-full transition-all duration-500 ease-out group-hover:opacity-90`}
            ...
            {/* Autres indicateurs */}
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-3 sm:p-4 col-span-1 md:col-span-1">
                ...
                <div className="space-y-2 sm:space-y-3">
                    ...
                    <div className="flex items-center justify-between group/item hover:bg-slate-50 -mx-2 px-2 py-1.5 sm:py-2 rounded-lg transition-colors">
                        ...
                        <span className="text-xs sm:text-sm font-bold text-slate-900">{operational.expeditions_attente_acceptation || 0}</span>
                    </div>
                </div>
            </div>
        </>

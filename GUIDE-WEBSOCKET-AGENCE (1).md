...
port/tunnel. On te communiquera :
...
|---|---|---|
| `created` | Une nouvelle expédition vient d'être créée dans ton agence (par toi ou un collègue connecté en même temps) | objet expédition complet, y compris `colis[]` (déjà créés et tarifés à ce stade) |
...
⚠️ Cas important pour `created` : c'est ce qui te permet d'afficher en temps réel
qu'un collègue de ton agence vient d'enregistrer une expédition (utile si
plusieurs agents sont connectés en même temps sur le même poste/agence) —
insère l'élément dans ta liste locale plutôt que de tout re-fetcher.
